jQuery(document).ready(function() {
  ...
  // ✅ FIX: Limpar hash IMEDIATAMENTE antes de qualquer inicialização
  // Evita que detectHash abra o painel ao fazer refresh
  ...
  jQuery(document).on( 'mouseover', '.__A__MainMenuV > li, .__A__MainMenuV li ul li', function() {
    ...
    theFold = $window.height() + wintop - 10;
  ...
  jQuery(".__A__Search_Input").doWithDelay("keyup", function(e) {
    ...
    let extra = {
      post_status: searchParams.get('status'),
      product: searchParams.get('product'),
      priority: searchParams.get('priority')
    };

    window.searchMe(extra);
  }, 200);
  ...
  function slideRevelWidth() {
    window.device_width = EnergyPlusGlobal.reactors_tweaks_window_size;
    window.device_width_notific = "450px";
  ...
  var trig =  jQuery('#slider').slideReveal({
    ...
    hidden: function(slider, trigger){
      ...
      if (1 === window.refreshOnClose && 1 === window.refreshOnClose2 && !window.isMobile) {
        // ✅ FIX: Reload para a página energyplus, não para a URL atual (que pode conter post.php)
  ...
  jQuery( "body" ).on( "click", '.trig', function(e) {
    if (!window.isMobile) {
      ...
      // ✅ FIX: Removido window.location.hash para evitar reabertura ao fazer refresh
  ...
  // ✅ FIX: detectHash não abre o painel — limpa o hash imediatamente ao carregar
