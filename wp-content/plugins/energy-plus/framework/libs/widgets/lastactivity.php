...
class Widgets__Lastactivity extends EnergyPlus_Widgets {
	...
	public static function run ( $args = array(), $settings = array() ) {
		...
		switch ($range) {
			...
			$__last_sessions = $wpdb->get_results(
				...
				WHERE type IN (1,2,4,5,6,7,10,17,18) AND date >= %s AND date <= %s ORDER BY request_id DESC LIMIT 100",
			...
			$__last_sessions = $wpdb->get_results(
				...
				SELECT DISTINCT(session_id)
				FROM {$wpdb->prefix}energyplus_requests
				WHERE type IN (1,2,4,5,6,7,10,17,18) AND month >= %d AND month <= %d AND date >= %s AND date <= %s ORDER BY request_id DESC LIMIT 50",
	...
	private static function get_session($session_ids) {
		...
		$query = "SELECT {$wpdb->prefix}energyplus_requests.* FROM {$wpdb->prefix}energyplus_requests WHERE type IN (1,2,4,5,6,7,10,17,18) AND month = %d AND (session_id='-1' ";
			...
			foreach ($__result AS $sess) {
				...
				switch ($sess->type) {
					...
					if (isset($cache[18][$sess->id])) {
						$category = $cache[18][$sess->id];
						++$cache[18][$sess->id]['cnt'];
						++$category['cnt'];
					} else {

						$object = get_term_by( 'id', $sess->id, 'product_tag', 'ARRAY_A' );
						...
						$cache[18][$sess->id] = $category;
					}
					$sessions[$sess->session_id]['views'][$sess->type.$sess->id] = array('time'=>$sess->date, 'details' => $category, 'type'=> 18);
