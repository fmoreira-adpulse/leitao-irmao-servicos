	...
	*/
	...
	public static function stats($coupon_id = 0, $args = array()) {
		...
		foreach ($orders AS $_orders) {
			...
			if (in_array($order_data['status'], ['failed', 'refunded'])) {
				continue;
			}
			...
			$coupon['total_sales']    += $order_data['total'] - $order_data['total_tax']- $order_data['shipping_total']- $order_data['shipping_tax'];
		}
	} else {
