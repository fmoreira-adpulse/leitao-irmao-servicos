	...
	*/
	...
	public static function filter($filter = false) {
		...
		if (EnergyPlus_Helpers::get('orderby')) {
			...
			if ('menu_order' === $filter['orderby']) {
					$filter['orderby'] = 'menu_order+title';
				}
						...
						$critical_stocks_ids = $wpdb->get_results(
							...
							*/
							...
							public static function extend_wp_query_where( $where, $wp_query ) {
								...
								if (isset($energy_q) && apply_filters('energyplus_search_products_where', false)) {
									$where .= apply_filters('energyplus_search_products_where', $energy_q);
								}
