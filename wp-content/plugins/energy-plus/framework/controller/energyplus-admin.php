...
class EnergyPlus_Admin {
  ...
  public static $wp_menu = array();
  public static $wp_sub_menu = array();
  ...
  */
  ...
  public static function init() {
    ...
    && (
      ...
      && (self::is_admin(null) || is_admin(null, 'shop_manager'))
      )
      {
        $userlist = apply_filters('energyplus_full_mode_by_user', []);

        if (isset($userlist[get_current_user_id()])) {
          if (true === $userlist[get_current_user_id()]) {
    ...
    */
    ...
    public static function admin_body_class( $classes ) {
      ...
      $classes .= ' energyplus-role-' . $user_role;
    ...
    */
    public static function get_menu( $args = array() ) {
      ...
      foreach (self::$wp_menu as $key => $value) {
      ...
      if (class_exists('WooCommerce')) {
        /* Badges */
        $order_count = wc_orders_count('on-hold') + wc_orders_count('processing') + wc_orders_count('pending');
        $output['energyplus-orders']['badge'] = apply_filters( 'woocommerce_menu_order_count', $order_count);
      }

      $output['energyplus-comments']['badge'] = intval(wp_count_comments()->moderated);
      $output['energyplus-customers']['badge'] =  intval(EnergyPlus::option('badge-customers', 0));
    ...
    public static function actions() {
      ...
      add_action( 'energyplus_order_details',           'EnergyPlus_Helpers::hook_order_details');
    }
    ...
    public static function filters() {
      ...
      // redirect comment after edit
      add_filter( 'comment_edit_redirect',              'EnergyPlus_Admin::filter_comment_edit_redirect', 10, 2 );
    }

    public static function ajaxs() {
      if (EnergyPlus_Helpers::ajax_enabled()) {
        // ajax
    ...
    public static function styles() {
      ...
      if ((self::is_full()) OR (!self::is_full() && self::is_energyplus())) {
        ...
        $colors_css = ":root{";
          ...
          if ('orders' === EnergyPlus_Helpers::get('segment')) {
            $order_statuses = wc_get_order_statuses();
            $order_colors = array('#039BE5','#4A148C','#CC66FF','#40E0D0','#6495ED','#28B463','#00FFFF','#808000','#DFFF00','#839192','#99FF33');
            $order_colors_pos = -1;
            foreach ($order_statuses AS $os=>$osv) {
              if (in_array($os, array('wc-processing','wc-completed','wc-cancelled','wc-refunded','wc-failed','wc-pending','wc-on-hold'))) {
                continue;
              }
              ++$order_colors_pos;
              $colors_css .= '.text-'. str_replace('wc-','', $os).' {color: '.$order_colors[$order_colors_pos%10].' !important} ';
              $colors_css .= '.bg-'. str_replace('wc-','', $os).' {background-color: '.$order_colors[$order_colors_pos%10].' !important} ';
              $colors_css .= '.badge-'. str_replace('wc-','', $os).' {background-color: '.$order_colors[$order_colors_pos%10].' !important} ';

            }
          }

          wp_add_inline_style('energyplus-theme', $colors_css);
          wp_add_inline_style('energyplus-theme', EnergyPlus::option('reactors-tweaks-custom-css',''));
          ...
          $JSvars['reactors_tweaks_quicklinks'] = intval(EnergyPlus::option('reactors-tweaks-settings-quicklinks', 0));

          if ('trash' !== EnergyPlus_Helpers::get('status', '') && 1 === intval(EnergyPlus_Helpers::get('pg', 1))) {
            $JSvars['reactors_tweaks_order_refresh']   = intval(apply_filters('energyplus-orders-refresh', EnergyPlus::option('reactors-tweaks-order-refresh', 0)));
          } else {
            $JSvars['reactors_tweaks_order_refresh']   =  0;
          }

          $JSvars['reactors_tweaks_order_notify']    = (is_array(EnergyPlus::option('reactors-tweaks-order-notify')))?array_keys(EnergyPlus::option('reactors-tweaks-order-notify', array())):array();
          $JSvars['reactors_tweaks_order_beep']      = EnergyPlus_Public . 'sounds/notification.mp3';
      ...
      public static function is_full() {
        ...
        if (isset($userlist[get_current_user_id()])) {
          return $userlist[get_current_user_id()];
        }
      ...
      */
      ...
      public static function is_admin( $user, $role = 'administrator' ) {
        ...
        $result =  in_array( $role, $user->roles, true );

        if (current_user_can('manage_options') && 'administrator' === $role) {
          $result = true;
        } else if (current_user_can('manage_woocommerce') && !current_user_can('manage_options') && 'shop_manager' === $role) {
          $result = true;
        }
      ...
      */
      ...
      public static function admin_head() {
        ...
        if (self!==top && window.parent.EnergyPlus_Window != null && window.parent.EnergyPlus_Window != undefined) {
          ...
          #newsletters_container, #mailpoet_editor, #forms_container, #subscribers_container, #settings_container { margin-top:100px } \
