  ...
  */
  ...
  public static function pulse() {
    ...
    switch (EnergyPlus_Helpers::post('t')) {
      ...
      $ID       = intval( EnergyPlus_Helpers::post('i', 0) );
      $category = get_term_by( 'id', $ID, 'product_tag', 'ARRAY_A' );
...
public static function get_visitor() {
  ...
  if ($visitor === 0) {
    ...
    if (isset($_COOKIE["energyplus-u"])) {
      ...
      $visitor_id = sanitize_key($_COOKIE["energyplus-u"]);
      ...
      if ($_session_id AND 32 === strlen($_session_id)) {
        ...
        if (32 === strlen($visitor_id) && 0<intval(get_current_user_id())) {
          global $wpdb;
          $wpdb->update($wpdb->prefix . 'energyplus_requests',
...
public static function check_gpdr_cookie() {
  // check cookies
  $cookies = apply_filters('energyplus_disable_tracker_cookies', []);
  ...
  foreach ($cookies AS $cookie_name=>$cookie_value) {
    if (isset($_COOKIE[$cookie_name])) {
      ++$count;
      if (stripos($_COOKIE[$cookie_name],$cookie_value) !== false) {
        return false;
      }
    }
  }

  if ($count === 0 && 0 < count($cookies)) {
    if (isset($_SERVER['HTTP_DNT']) && intval($_SERVER['HTTP_DNT']) === 1) {
      return false;
    }
  }
...
public static function delete_user_tracker_data() {
  global $wpdb;
  if (isset($_COOKIE["energyplus-session"])) {
    $visitor = self::get_visitor();
    if (!empty($visitor)) {
      $wpdb->delete( $wpdb->prefix.'energyplus_requests', array( 'visitor' => $visitor ) );
    }

  }

  if (isset($_COOKIE["energyplus-u"])) {
    setcookie("energyplus-u", '', time()-(365*24*60*60), COOKIEPATH, COOKIE_DOMAIN);
  }

  if (isset($_COOKIE["energyplus-session"])) {
    setcookie("energyplus-session", '', time()-(365*24*60*60), COOKIEPATH, COOKIE_DOMAIN);
  }
