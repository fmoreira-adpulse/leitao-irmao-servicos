  ...
  public static function ajax() {
    ...
    switch ($do) {
      ...
      if ('approve' === $state) {
        $undostate = 'approve';
        $result    = wp_set_comment_status( $id, 'approve' );
        $message   = esc_html__('Approved', 'energyplus');
      }

      if ('unapprove' === $state) {
        $undostate = 'unapprove';
        $result    = wp_set_comment_status( $id, 'hold' );
        $message   = esc_html__('Unapproved', 'energyplus');
      }

      if ('restore' === $state) {
        $undostate = 'restore';
        $result    = wp_untrash_comment( $id );
        $message   = esc_html__('Done', 'energyplus');
      }

      if ('spam' === $state) {
        $undostate = 'spam';
        $result    = wp_spam_comment( $id );
        $message   = esc_html__('Done', 'energyplus') . '&mdash; <a class="__A__AjaxButton" href="javascript:;" data-id="'.$id.'" data-do="status" data-state="unspam">'.esc_html__('Undo', 'energyplus').'</a>';
      }
      ...
      if ('trash' === $state) {
        $undostate = 'trash';
        $result    = wp_delete_comment( $id );
        $message   = esc_html__('Done', 'energyplus') . ' &mdash; <a class="__A__AjaxButton" href="javascript:;" data-id="'.$id.'" data-do="status" data-state="untrash">'.esc_html__('Undo', 'energyplus').'</a>';
      }
      ...
      if (TRUE === $result) {
        return EnergyPlus_Ajax::success(esc_html__('Done', 'energyplus'), array('id'=> $id, 'message' => $message, 'state' => $undostate ));
      } else {
        return EnergyPlus_Ajax::error( esc_html__('Error', 'energyplus') );
      }
