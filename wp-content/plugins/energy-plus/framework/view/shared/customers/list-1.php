      ...
      <div class="btnA __A__Item collapsed" id="item_<?php echo  esc_attr($customer['id'])?>" data-toggle="collapse" data-target="#item_d_<?php echo  esc_attr($customer['id'])?>" aria-expanded="false" aria-controls="item_d_<?php echo esc_attr($customer['id'])?>">
        <div class="liste  row d-flex align-items-center">
          ...
          <div class="col-7 col-sm-3 __A__Col_Name">
            <p class="energyplus-orders--name eix-quick" href="<?php echo admin_url( "user-edit.php?user_id=" . esc_attr($customer['id'])); ?>">
          ...
          <div class="__A__Item_Details">
            <div class="row">
              ...
              <div class="col-sm-2 __A__Customer_Details_Actions">
                <?php do_action('energyplus_before_customer_actions', intval($customer['id'])) ?>
                ...
                <?php if (current_user_can('delete_users')) { ?><a href="<?php echo wp_nonce_url( "users.php?action=delete&user=" . esc_attr($customer['id']), 'bulk-users' ); ?>" class="__A__HideMe __A__StopPropagation text-danger trig"><?php esc_html_e('Delete', 'energyplus'); ?></a><?php } ?>
                <?php do_action('energyplus_after_customer_actions', intval($customer['id'])) ?>
              </div>
            </div>
          </div>
        </div>
