    ...
    <table class="table">
      <?php foreach ($result AS $r) { ?>
        <tr>
          ...
          <?php } elseif ( '18' === $r['type']) { ?>
            <?php echo get_term_by( 'id', $r['id'], 'product_tag' )->name; ?>&nbsp; <span class="badge-pill text-uppercase badge-secondary __A__Cat"><?php esc_html_e('Tag', 'energyplus'); ?></span>
        <?php } else { ?>
          <?php echo get_term_by( 'id', $r['id'], 'product_cat' )->name; ?>&nbsp; <span class="badge-pill text-uppercase badge-secondary __A__Cat"><?php esc_html_e('Category', 'energyplus'); ?></span>
        <?php } ?>
          </td>
          <td class="text-right">
