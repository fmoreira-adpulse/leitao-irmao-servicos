...
<?php if (!$ajax) {  ?>
  ...
  <div id="energyplus-orders-1">
    ...
    <ul class="adpulse-search-filters" data-tab="status">
    ...
    <ul class="adpulse-search-filters" data-tab="product">
    <li><a class="__A__Button1<?php EnergyPlus_Helpers::selected('product')?>" href="<?php echo EnergyPlus_Helpers::admin_page('orders', array());  ?>"><?php esc_html_e('All Services', 'energyplus'); ?> <?php echo esc_html($list['statuses_count']['any']) ?></a></li>
    <?php foreach ($list['products'] as $product_key => $product) { ?>
          <li><a class="__A__Button1<?php EnergyPlus_Helpers::selected('product', strval($product_key))?>" href="<?php echo EnergyPlus_Helpers::admin_page('orders', array('product' => $product_key)); ?>"><?php echo esc_attr($product) ?></a></li>
          <?php
        } ?>
    </ul>

    <!-- Priority filter options -->
    <ul class="adpulse-search-filters" data-tab="priority">
    <li><a class="__A__Button1<?php EnergyPlus_Helpers::selected('priority')?>" href="<?php echo EnergyPlus_Helpers::admin_page('orders', array());  ?>"><?php esc_html_e('All Priorities', 'energyplus'); ?> <?php echo esc_html($list['statuses_count']['any']) ?></a></li>
      <li><a class="__A__Button1<?php EnergyPlus_Helpers::selected('priority', '1')?>" href="<?php echo EnergyPlus_Helpers::admin_page('orders', array('priority' => "1")); ?>"><?php echo esc_attr("Prioritário") ?></a></li>
      <li><a class="__A__Button1<?php EnergyPlus_Helpers::selected('priority', '0')?>" href="<?php echo EnergyPlus_Helpers::admin_page('orders', array('priority' => "0")); ?>"><?php echo esc_attr("Sem Prioridade") ?></a></li>
    </ul>
        ...
        <?php  foreach ($orders AS $order_group) {  ?>
          ...
          <div class="__A__Orders_Container">
            <?php foreach ($order_group['orders'] AS $order) {  ?>
              <div class="btnA __A__Item collapsed <?php do_action("add_class_to_order_div", $order) ?>"  data-toggle="collapse" data-target="#order_<?php echo esc_attr($order['id'])?>" aria-expanded="false" aria-controls="order_<?php echo esc_attr($order['id'])?>"  id="item_<?php echo esc_attr($order['id'])?>" rel-status="<?php echo esc_attr($order['status'])?>">
                <div class="liste  row d-flex align-items-center">
                  ...
                  <div class="text-center d-none d-sm-inline __A__Order_No" style="display:inline-block !important; min-width:25%; padding: 4px 0px;" data-colname="<?php esc_html_e('Order No: ', 'energyplus'); ?>">
                    <span class="__A__Order_No __A__Strong eix-quick"  href="<?php echo admin_url( 'post.php?post=' . esc_attr($order['id']). '&action=edit&energyplus_hide' );?>"><?php echo esc_attr($order['std'] instanceof WC_Order? $order['std']->get_order_number() : '')?></span>
                    <?php if ($order['has_priority']) { ?>
                      <div class="has_priority"><?= __('Prioritário') ?></div>
                    <?php } ?>
                  </div>
                  <div class="col col-sm-2 col-md-1 energyplus-orders--item-badge text-center __A__Col_3">
                    ...
                    <div class="__A__Col_Name col-7 col-sm-2">
                      <p class="energyplus-orders--name eix-quick" href="<?php echo admin_url( 'post.php?post=' . esc_attr($order['id']). '&action=edit&energyplus_hide' );?>">
                      ...
                      <p class="energyplus-orders--seller eix-quick font-weight-bold"  href="<?php echo admin_url( 'post.php?post=' . esc_attr($order['id']). '&action=edit&energyplus_hide' );?>">
                        <?= __('Vendedor') ?>
                      </p>
                      <p class="energyplus-orders--seller eix-quick"  href="<?php echo admin_url( 'post.php?post=' . esc_attr($order['id']). '&action=edit&energyplus_hide' );?>">
                        <?= get_order_seller_name($order['id']) ?>
                      </p>
                    </div>
                  ...
                  <div class="row __A__Order_Items">
                    <div class="col-md-4 col-sm-6">
                            ...
                            if ($formatted_meta_data) {  ?>
                              <div class="__A__Order_Details_Variation">
                                ...
                                foreach ($formatted_meta_data AS $meta) {
                                  echo '<strong>' . esc_html($meta->display_key). '</strong>: <span class="badgex badge-pillx badge-blackx"> ' . wp_kses($meta->display_value, ['p' => [], 'br' => []]) . '</span> &nbsp; &nbsp;<br> ';
                                }
                                ?>
                              </div>
                            <?php } ?>
                      ...
                      <div class="col-md-4 col-sm-5 __A__StopPropagation __A__Order_Address">
                        ...
                        <?php do_action('energyplus_after_order_addresses', intval($order['id']), $order) ?>
...
if (empty($count_status) || !isset($list['statuses_count'][$count_status])) {
  $count_status = 'any';
}
        ?>
<?php
$per_page = absint(EnergyPlus::option('reactors-tweaks-pg-orders', 20));
$current_page = max(1, intval(EnergyPlus_Helpers::get('pg', 1)));
$total = $list['statuses_count'][$count_status];
$from = (($current_page - 1) * $per_page) + 1;
$to = min($current_page * $per_page, $total);
echo '<p style="text-align:left; margin-top: 0px; margin-left: 5px; font-weight:bold; color:#666; padding: 5px 0;">Ordens ' . $from . ' a ' . $to . ' de ' . $total . '</p>';
?>
        <?php echo EnergyPlus_View::run( 'core/pagination', array( 'count' => $list['statuses_count'][$count_status], 'url'=>EnergyPlus_Helpers::admin_page('orders', array_filter(array('status'=>EnergyPlus_Helpers::get('status', EnergyPlus_Helpers::post('status', '')), 'product'=>EnergyPlus_Helpers::get('product'), 'priority'=>EnergyPlus_Helpers::get('priority')), function($v) { return $v !== null && $v !== '' && $v !== '0' && $v !== 0; })), 'per_page'=> absint(EnergyPlus::option('reactors-tweaks-pg-orders', 20)), 'page' => intval ( EnergyPlus_Helpers::get( 'pg', 0 ) ) )); ?>
      </div>
    </div>
  <?php } ?>