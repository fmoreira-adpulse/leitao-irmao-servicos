...
abstract class THWEPOF_Admin_Form {
	...
	public function render_form_field_element($field, $atts = array(), $render_cell = true){
		if($field && is_array($field)){
			...
			if($render_cell){
				...
				<td <?php echo wp_kses_post($input_cell_props); ?> >
					<?php 
					// phpcs:ignore WordPress.Security.EscapeOutput.OutputNotEscaped -- Admin form HTML generated by plugin
					echo $field_html; 
					?>
				</td>
				<?php
	...
	public function render_form_fragment_tooltip($tooltip = false){
		...
		if($tooltip){
			$tooltip_html = '<a href="javascript:void(0)" title="'. esc_attr($tooltip) .'" class="thwepof_tooltip"><img src="'. THWEPOF_URL.'admin/assets/css/help.png" title=""/></a>';
		}
		?>
		<td style="width: 26px; padding:0px;"><?php echo wp_kses_post($tooltip_html); ?></td>
		<?php
	}
	...
	public function render_form_fragment_rules($type="field"){
		...
		<tr>
        	<td class="">
                <?php esc_html_e('Show if all below conditions are met.', 'woo-extra-product-options'); ?>
            </td>
        </tr>
        <tr>                
            <td colspan="6">
                                                        ...
                                                        <tr class="thwepo_condition">
                                                            <td class="operand-type">
                                                                <select name="i_rule_subject" onchange="thwepofRuleOperandTypeChangeListner(this)">
                                                                    <option value=""></option>
                                                                    <option value="product"><?php esc_html_e('Product', 'woo-extra-product-options'); ?></option>
                                                                    <option value="category"><?php esc_html_e('Category', 'woo-extra-product-options'); ?> </option>
                                                                    <option value="tag"><?php esc_html_e('Tag', 'woo-extra-product-options'); ?></option>
                                                                </select>
                                                            </td>
                                                            <td class="operator">
                                                                <select name="i_rule_comparison">
                                                                    <option value=""></option>
                                                                    <option value="equals"><?php esc_html_e('Equals to/ In', 'woo-extra-product-options'); ?></option>
                                                                    <option value="not_equals"><?php esc_html_e('Not Equals to/ Not in', 'woo-extra-product-options'); ?></option>
                                                                </select>
                                                            </td>
	...
	public function render_field_form_fragment_product_list(){
        ...
        <select multiple="multiple" name="i_rule_value" data-placeholder="<?php esc_attr_e('Click to select products', 'woo-extra-product-options'); ?>" class="thwepof-enhanced-multi-select1 thwepof-operand thwepof-product-select" style="width:98%;" value="">
        </select>
        </div>
        <?php
	}
	...
	public function render_field_form_fragment_category_list(){		
		...
		if(!empty($categories)){
	        ...
	        <select multiple="multiple" name="i_rule_value" data-placeholder="<?php esc_attr_e('Click to select categories', 'woo-extra-product-options'); ?>" class="thwepof-enhanced-multi-select thwepo-operand" style="width:98%;" value="">
				<?php 	
	...
	public function render_field_form_fragment_tag_list(){		
		...
		if(!empty($tags)){
	        ...
	        <select multiple="multiple" name="i_rule_value" data-placeholder="<?php esc_attr_e('Click to select tags', 'woo-extra-product-options'); ?>" class="thwepof-enhanced-multi-select thwepo-operand" style="width:98%;" value="">
				<?php 	
	                foreach($tags as $tag){
	...
	public function load_product_terms($taxonomy, $only_slug = false){
		$product_terms = array();
		$pterms = get_terms( array( 'taxonomy' => $taxonomy, 'orderby' => 'count', 'hide_empty' => false ) );
