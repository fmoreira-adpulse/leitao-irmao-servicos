...
/**
 * Class Wtcrp_Bfcm_Twenty_Twenty_Four
 *
 * This class is responsible for displaying and handling the Black Friday and Cyber Monday CTA banners for 2024.
 */
if ( ! class_exists( 'Wtcrp_Bfcm_Twenty_Twenty_Four' ) ) {

    class Wtcrp_Bfcm_Twenty_Twenty_Four {
        
        private $banner_id = 'wtcrp-bfcm-twenty-twenty-four';
        private static $banner_state_option_name = "wtcrp_bfcm_twenty_twenty_four_banner_state"; // Banner state, 1: Show, 2: Closed by user, 3: Clicked the grab button, 4: Expired
        private $banner_state = 1;
        private static $show_banner = null;
        private static $ajax_action_name = "wtcrp_bcfm_twenty_twenty_four_banner_state";
        ...
        public function enqueue_styles_and_scripts() {
            wp_enqueue_style( $this->banner_id.'-css', plugins_url( 'admin/css/wtcrp-bfcm-twenty-twenty-four.css', dirname(__FILE__) ), array(), self::$banner_version, 'all' );
            $params = array(
                'ajax_url' => admin_url( 'admin-ajax.php' ),
                'nonce' => wp_create_nonce( 'wtcrp_bfcm_twenty_twenty_four_banner_nonce' ),
                'action' => self::$ajax_action_name,
                'cta_link' => self::$promotion_link,
            );
            wp_enqueue_script(
                $this->banner_id . '-js', 
                plugins_url( 'admin/js/wtcrp-bfcm-twenty-twenty-four.js', dirname(__FILE__) ), 
                array( 'jquery' ), 
                self::$banner_version, 
                false
            );         
            wp_localize_script( $this->banner_id.'-js', 'wtcrp_bfcm_twenty_twenty_four_banner_js_params', $params );
        }

        public function show_banner () {
            if ( $this->is_show_banner() ) {
                ?>
                    <div class="wtcrp-bfcm-banner-2024 notice is-dismissible">
                        <div class="wtcrp-bfcm-banner-body">
                            <div class="wtcrp-bfcm-banner-body-img-section">
                                <img src="<?php echo esc_url( CRP_PLUGIN_URL . 'admin/img/black-friday-2024.svg', dirname(__FILE__) ); ?>" alt="<?php esc_html_e('Black Friday Cyber Monday 2024', 'wt-woocommerce-related-products'); ?>">
                            </div>
                            <div class="wtcrp-bfcm-banner-body-info">
                                <div class="wtcrp-never-miss-this-deal">
                                    <p><?php esc_html_e( 'Never Miss This Deal', 'wt-woocommerce-related-products' ); ?></p>
                                </div>
                                <div class="info">
                                    <p>
                                        <?php echo sprintf( esc_html__( 'Your Last Chance to Avail %1$s on WebToffee Plugins. Grab the deal before it\'s gone!', 'wt-woocommerce-related-products' ), 
                                            '<span>30% ' . esc_html__( 'OFF', 'wt-woocommerce-related-products' ) . '</span>' );
                                        ?>
                                    </p>
                                </div>
                                <a href="<?php esc_url(self::$promotion_link); ?>" class="wtcrp-bfcm-cta-button" target="_blank">
                                    <div class="wtcrp-bfcm-banner-body-button">
                                        <?php esc_html_e( 'View plugins', 'wt-woocommerce-related-products' ); ?> 
        ...
        public function is_show_banner () {
            ...
            self::$show_banner = false;
            if ( 'woocommerce_page_wt-woocommerce-related-products' === $screen_id ) {
    	...
    	public function update_banner_state() {
    		check_ajax_referer( 'wtcrp_bfcm_twenty_twenty_four_banner_nonce' );
    		if ( isset( $_POST['wtcrp_bfcm_twenty_twenty_four_banner_action_type'] ) ) {
	            
	            $action_type = absint( sanitize_text_field( $_POST['wtcrp_bfcm_twenty_twenty_four_banner_action_type'] ) );
    ...
    new Wtcrp_Bfcm_Twenty_Twenty_Four();
}