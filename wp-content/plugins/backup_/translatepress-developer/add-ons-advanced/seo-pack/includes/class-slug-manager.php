...
class TRP_IN_SP_Slug_Manager {
    ...
    public function is_redirect_needed( $current_url_with_domain ){
        ...
        if( $TRP_LANGUAGE != $this->settings['default-language'] ) {
            ...
            $location = $this->replace_slugs_in_url_path( $current_url_with_domain, $slug_pairs );
            ...
            $url_last_word              = explode( '/', $url_delete_last_slash );

            if ( 'feed' != end( $url_last_word ) ) {
                if ( $current_url_with_domain != $location ) {
                    $this->redirect_to_url = $location;
...
//            }
    ...
    public function get_slug_translated_url_for_language( $target_url, $source_url, $target_language ){
        ...
        $abs_home_path = untrailingslashit(strval($abs_home_object->getPath()));

        $path_no_lang_slug = trp_remove_prefix($abs_home_path, $this->get_path_no_lang_slug_from_url( $target_url ));
        ...
        $translated_path = trp_remove_prefix($abs_home_path, $this->replace_slugs_in_url_path( $path_no_lang_slug, $slug_pairs ));
            ...
            if ( !apply_filters( 'trp_is_admin_link_for_request_uri', false, $current_url_with_domain ) && !apply_filters( 'trp_is_rest_api', false, $sanitized_request_uri) ) {
                if ( !apply_filters( 'trp_is_form_for_request_uri', $is_form, $current_url_with_domain ) )
                    if ( $this->is_redirect_needed( $current_url_with_domain ) ){
                        // redirect will happen on 'template_redirect'
                        return;
                    }
